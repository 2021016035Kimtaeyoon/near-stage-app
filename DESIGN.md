...
주조색은 로고에서 그대로 가져온 밝은 노랑(금색) 하나이며, 크림슨은 히어로 전용 무드 색, 나머지는 무채색과 기능색(성공/경고/위험/정보)입니다.

> **기본 테마는 라이트(흰 배경)입니다.** 아래 무채색 값은 다크 테마 기준이고, 라이트 테마 값은 `src/index.css`의 `:root` 블록에 있습니다. 마이페이지에서 토글할 수 있으며, 랜딩 히어로만 항상 다크로 고정됩니다.

### Brand
- **Gold 500** (`#FFC42E`): 기준 브랜드색. 기본 CTA 배경, 활성 탭 인디케이터, 강조 숫자, 포커스 링.
- **Gold 400** (`#FFDA6B`) / **Gold 600** (`#E9A600`): 밝은/어두운 톤 보정용(hover, 보조 강조).
- **Gold Ink** (`#241A00`): 금색 '채움' 위에 올라가는 텍스트 전용 색(대비 10.8:1). **금색 배경 위에 흰 글자를 쓰지 않는다(대비 부족).**
- **Gold Text** (`#8A6100` 라이트 / `#FFC42E` 다크): 배경 위에 금색 '글자·아이콘'으로 쓸 때 전용(흰 배경 대비 5.5:1). **밝은 노랑(`gold-500`)을 흰 배경 위 글자색으로 쓰지 않는다(대비 1.7:1).**
