...
## 커튼 — 좌우 분할 개막 + 실제 벨벳 사진

절차적으로 그린 SVG 주름은 아무리 다듬어도 2D 줄무늬처럼 보여서, **실제 벨벳 커튼 사진**
(`public/curtain-velvet.jpg`, 2400×1641 / 275KB)으로 교체했습니다.
...
`right center`(오른쪽 절반)를 보여줍니다. 닫혀 있을 때 두 폭이 정확히 원본 한 장으로 맞물립니다.

**사진 위 오버레이 5겹** (패널의 자식이라 scaleX/x와 함께 통째로 변형됨):
① 안쪽에서 비추는 무대 조명 radial → ② 안쪽 선단 3px 하이라이트 띠 → ③ 바깥쪽 끝 암부
→ ④ 위(배튼 그늘)/아래(바닥 그늘) 세로 그라데이션 → ⑤ 옅은 비네트(모바일 생략).
②와 중앙 이음새 그림자는 `mask-image`로 아래쪽 62~82% 구간에서 사라집니다 — 바닥까지 내려오면
사진 위에 자를 대고 그은 선처럼 보이기 때문입니다.

**밸런스**도 같은 사진을 씁니다. 스캘럽 실루엣을 SVG `<mask>`로 만들고 그 안을 사진 윗단
(`<image preserveAspectRatio="xMidYMin slice">`)으로 채운 뒤, 조명보다 위라는 걸 나타내는 어두운
세로 그라데이션을 덮었습니다. 화면이 넓어지면 스캘럽 개수만 늘어납니다(절대 px 패턴).

DOM에서 두 패널의 `background-position`이 각각 `0% 50%` / `100% 50%`, `background-size`가
`200% 100%`인 것과 밸런스 `<image href="/curtain-velvet.jpg">`를 확인했습니다.
