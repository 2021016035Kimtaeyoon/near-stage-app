import { Bell, BellOff, BellPlus, Trash2 } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import { useAuthStore } from '@/hooks/useAuth'
import { useSavedSearches } from '@/hooks/useSavedSearches'
import { cn } from '@/lib/cn'
import { useAppStore } from '@/store/useAppStore'
import { toast } from '@/store/useToast'

/**
 * 관심 조건 목록.
 *
 * 행을 누르면 그 장르를 홈 필터에 얹고 홈으로 보냅니다. 종은 알림 on/off,
 * 휴지통은 삭제입니다. 마이페이지와 데스크톱 사이드바가 함께 씁니다.
 *
 * ★ 예전에는 이 화면이 브라우저 메모리를 읽었습니다. 새로고침하면 목록이
 *   사라졌고 알림은 오지 않았습니다. 이제 DB(saved_searches)를 읽습니다.
 */
export function SavedSearchPanel({ compact = false }: { compact?: boolean }) {
  const navigate = useNavigate()
  const userId = useAuthStore((s) => s.userId)
  const setFilter = useAppStore((s) => s.setAudienceFilter)
  const saved = useSavedSearches()

  const alertCount = saved.data.filter((s) => s.alertOn).length

  return (
    <section className={cn('card overflow-hidden', compact ? 'mb-0' : 'mb-4')}>
      <div className="flex items-start justify-between gap-3 px-4 pb-2.5 pt-3.5">
        <div className="min-w-0">
          <h2 className="flex items-center gap-1.5 text-[13px] font-bold">
            <BellPlus size={15} className="text-gold-text" />
            관심 조건
          </h2>
          <p className="mt-0.5 text-2xs leading-snug text-ink-3">
            고른 장르의 무대가 새로 열리면 알려드려요.
          </p>
        </div>
        {saved.data.length > 0 && (
          <span className="tnum shrink-0 rounded-full bg-surface-2 px-2 py-1 text-2xs font-bold text-ink-2">
            알림 {alertCount}
          </span>
        )}
      </div>

      {!userId ? (
        <p className="border-t border-border px-4 py-5 text-center text-xs leading-relaxed text-ink-3">
          로그인하면 관심 장르를 저장하고
          <br />
          새 무대가 열릴 때 알림을 받을 수 있어요.
        </p>
      ) : saved.loading ? (
        <div className="border-t border-border p-4">
          <div className="h-12 animate-pulse rounded-xl bg-surface-2" />
        </div>
      ) : saved.error ? (
        <p className="border-t border-border px-4 py-5 text-center text-xs leading-relaxed text-danger">
          {saved.error}
        </p>
      ) : saved.data.length === 0 ? (
        <p className="border-t border-border px-4 py-5 text-center text-xs leading-relaxed text-ink-3">
          아직 저장한 장르가 없어요.
          <br />
          홈에서 장르를 고른 뒤{' '}
          <span className="font-bold text-ink-2">이 장르 저장</span>을 눌러보세요.
        </p>
      ) : (
        <ul className="divide-y divide-border border-t border-border">
          {saved.data.map((s) => (
            <li key={s.id} className="flex items-center gap-1">
              <button
                onClick={() => {
                  setFilter({ genres: s.genres })
                  navigate('/audience/home')
                }}
                className="tap min-w-0 flex-1 px-4 py-3 text-left"
              >
                <span className="block truncate text-[13px] font-bold">{s.name}</span>
                <span className="mt-0.5 block truncate text-2xs text-ink-3">
                  {s.genres.length === 0 ? '장르 상관없이' : s.genres.join(' · ')}
                </span>
              </button>

              <button
                onClick={() =>
                  void (async () => {
                    const next = !s.alertOn
                    const err = await saved.toggleAlert(s.id, next)
                    if (err) toast('바꾸지 못했어요', 'error', err)
                    else toast(next ? '알림을 켰어요' : '알림을 껐어요', 'default', s.name)
                  })()
                }
                aria-label={s.alertOn ? `${s.name} 알림 끄기` : `${s.name} 알림 켜기`}
                aria-pressed={s.alertOn}
                className={cn(
                  'flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition-colors',
                  s.alertOn ? 'bg-gold-500 text-gold-ink' : 'bg-surface-2 text-ink-3',
                )}
              >
                {s.alertOn ? <Bell size={15} /> : <BellOff size={15} />}
              </button>

              <button
                onClick={() =>
                  void (async () => {
                    const err = await saved.remove(s.id)
                    if (err) toast('삭제하지 못했어요', 'error', err)
                    else toast('관심 조건을 삭제했어요', 'default', s.name)
                  })()
                }
                aria-label={`${s.name} 삭제`}
                className="mr-3 flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-ink-3 active:bg-surface-2"
              >
                <Trash2 size={15} />
              </button>
            </li>
          ))}
        </ul>
      )}
    </section>
  )
}
