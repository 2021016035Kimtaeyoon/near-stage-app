...
export function HomeMap() {
  ...
  //   ★ '매일 공연'도 여기서 빠집니다 — 날짜를 좁히지는 못하지만 모르는 게
  //     아니라 아는 것입니다. scheduleOf 가 daily 를 돌려주므로 자동입니다.
      ...
      <AnimatePresence>
        {view === 'map' && selected && (
          <motion.div
            ...
            className="elevation-float absolute left-1/2 z-40 -translate-x-1/2 overflow-hidden rounded-2xl border border-border bg-surface/97"
            style={{ bottom: snap === 0 ? 150 : 402 }}
          >
            <ShowMiniCard
              item={selected}
              nowIso={nowIso}
