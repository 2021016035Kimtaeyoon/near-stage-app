...
function ChartTooltip({ active, payload }: { active?: boolean; payload?: TooltipPayload[] }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-white px-3 py-2 text-xs shadow-lg">
      ...
      <p className={d.hadShow ? 'mt-0.5 font-semibold text-[#FF5560]' : 'mt-0.5 text-ink-3'}>
  ...
  return (
    <div className="card p-4">
      <div className="mb-1 flex items-center gap-1.5">
        <TrendingUp size={15} className="text-[#FF5560]" />
      ...
      <div className="mt-3 rounded-xl bg-surface-2 p-3">
        <p className="text-[13px] font-semibold leading-relaxed">
          ...
          <span className="text-[#FF5560]">{pct > 0 ? `${pct}%` : `${Math.abs(pct)}%`}</span>{' '}
          {pct >= 0 ? '높습니다' : '낮습니다'}.
        </p>
      </div>
    </div>
